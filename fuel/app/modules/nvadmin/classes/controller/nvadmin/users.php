...
class Controller_NVAdmin_Users extends \NVAdmin\Controller_NVAdmin
{
	...
	{        
        ...
        $this->data['page_values']['breadcrumbs'] = array(\Lang::get('nvadmin.private.users.shared.name_plural') => '');
		...
		if($val->run())
		{
            ...
            try
            {
                $user_count = \Model_User::find()
                    ...
                    ->where('id', '!=', $id)
                    ->count();
                if($user_count > 0) { throw new \SimpleUserUpdateException('Email already exists.', 2); }
            ...
            catch(\SimpleUserUpdateException $e) { $errors['update'] = \Lang::get('nvadmin.exception.email_already_exists_exception'); }
            catch(\SimpleUserWrongPassword $e)   { $errors['update'] = \Lang::get('nvadmin.exception.old_password_invalid_exception'); }
		}
		else
		{
			$errors = $val->error();
